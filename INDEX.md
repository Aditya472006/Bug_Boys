...
## 🎯 Complete Water Governance Dashboard Package
...
### 🎯 **GENERATED FILES** (Created after setup)
...
│
├─→ 2. GENERATE: python generate_models.py
│       Creates: random_forest.pkl, scaler.pkl
│
...
│
├── Interactive Maps
│   ├── Overview Map (all villages)
│   └── Route Optimization (top 5 priority)
│
├── Priority Ranking Table
│   └── Sortable village rankings
│
├── Village Simulation Panel
│   ├── Village Selector
│   ├── WSI Progress Bar
...
└── Export Options
    ...
    └── Download High-Priority Villages
```
...
├─ Output: WSI (0-1)
    ↓
[Priority Scoring]
├─ priority = (WSI × 0.70) + (population × 0.30)
    ↓
...
### Priority Score
- **Formula**: (WSI × 0.70) + (Normalized Population × 0.30)
- **Purpose**: Determine resource allocation order
- **Usage**: Sort villages by urgency

### Tankers Required
- **Formula**: ⌈(Population × Annual Need - Current Storage) / 10,000⌉
...
├── Requires: random_forest.pkl
├── Requires: scaler.pkl
├── Imports: pandas, numpy, streamlit, folium, sklearn
└── Optional: config.py (for advanced customization)
